...
**PWA layer** (`manifest.webmanifest`, `sw.js`, icon files): makes the app installable ("Add to Dock"/home screen) when served over http(s). `sw.js` precaches a fixed asset list under a cache name that is version-stamped by hand (`normalizador-bibliografico-v1.7`) — **this string must be bumped on every release that changes cached assets**, or returning users keep getting the stale cached version indefinitely.
